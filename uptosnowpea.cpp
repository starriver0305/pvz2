...
UptoSnowPea::UptoSnowPea()
{
    movie=new QMovie(":/new/prefix1/pictures/card/snowpea.png");
    movie->start();
}

void UptoSnowPea::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event);
...
void UptoSnowPea::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event);
    point3=QCursor::pos();
...
void UptoSnowPea::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    Q_UNUSED(event);
    ...
    foreach(QGraphicsItem* item,items){
        ...
        if(shooter){
            SnowPea *pea=new SnowPea(shooter->x,shooter->y);
        ...
        if(repeater){
            SnowPea *pea=new SnowPea(repeater->x,repeater->y);
